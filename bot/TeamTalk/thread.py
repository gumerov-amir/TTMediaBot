...
class TeamTalkThread(Thread):
    def __init__(self, bot: Bot, ttclient: TeamTalk) -> None:
    ...
    def run(self) -> None:  # noqa: C901, PLR0912, PLR0915
        ...
        while not self._close:
            ...
            if (
                ...
                EventType.CON_FAILED,
                EventType.CON_LOST,
                EventType.MYSELF_KICKED,
            ):
    ...
    def parse_event(self, event: Event) -> tuple[Any, ...]:
        if event.event_type in (
            ...
            return (event.user,)
        if event.event_type == EventType.USER_LEFT:
            return (event.source, event.user)
        if event.event_type == EventType.USER_TEXT_MESSAGE:
    ...
    def run_event_handler(self, event: Event) -> None:
        try:
            event_handler: Callable[..., None] | None = getattr(
                self.event_handlers,
